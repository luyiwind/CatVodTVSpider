...
public class XPath extends Spider {
                ...
                if (!rule.getHomeVodNode().isEmpty()) {
                    try {
                        ...
                        for (int i = 0; i < vodNodes.size(); i++) {
                            ...
                            //vodNodes.get(i).selOne("/span[3]/text()").asString().trim()
                            String rate = "";
                            if (!rule.getHomeVodRate().isEmpty()) {
                                try {
                                    rate = vodNodes.get(i).selOne(rule.getHomeVodRate()).asString().trim();
                                } catch (Exception e) {
                                    SpiderDebug.log(e);
                                }
                            }
    ...
    public String detailContent(List<String> ids) {
        try {
            ...
            if (!rule.getHomeVodRate().isEmpty()) {
                try {
                    rate = vodNode.selOne(rule.getHomeVodRate()).asString().trim();
                } catch (Exception e) {
                    SpiderDebug.log(e);
                }
            }
